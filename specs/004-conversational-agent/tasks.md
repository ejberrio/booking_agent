...
> **Estado: IMPLEMENTADO (2026-06-26).** 49 tests en verde (10 nuevos del agente con FakeLLM + ChannelManager falso), ruff limpio. Migración `248395fee3e4_agent_action.py` encadenada al head. El test legado `test_chat_demo_mode` se retiró (el `/chat` placeholder fue reemplazado por el agente real). SSE en `/chat/stream`.
...
- [x] T001 [P] Crear paquete `app/agent/` (con `__init__.py`) en `apps/api/app/`
...
- [x] T002 Capa de tool-calling: `chat_with_tools(messages, tools, model)` vía LiteLLM + `Protocol` LLM inyectable (para FakeLLM) en `apps/api/app/llm/client.py`
- [x] T003 [P] Modelo `AgentAction` + enum `AgentActionStatus` en `apps/api/app/models/agent.py` y `apps/api/app/models/enums.py` (registrar en `app/models/__init__.py`)
- [x] T004 [P] System prompt (no inventar datos; proponer y confirmar) en `apps/api/app/agent/prompts.py`
- [x] T005 Registro de herramientas (`ToolSpec`) + herramientas de **lectura** (`get_calendar`, `get_history`) mapeadas a la feature 003, en `apps/api/app/agent/tools.py`
- [x] T006 Migración Alembic para `agent_action` en `apps/api/migrations/versions/`
...
- [x] T007 [P] [US1] Test: consulta → llama `get_calendar` y responde datos reales (no inventa); sin `LLMConfig` → mensaje claro sin acciones, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 1

- [x] T008 [US1] Orquestador base `run_turn` (persiste mensaje, arma prompt + historial, loop de tool-calling con herramientas de lectura; sin `LLMConfig` → mensaje claro) en `apps/api/app/agent/orchestrator.py`
- [x] T009 [US1] Endpoint `POST /chat` (no streaming) → `AgentReply`, en `apps/api/app/api/routes/chat.py`
...
- [x] T010 [P] [US2] Tests: escritura propone (proposed, no aplica); `confirm_pending` aplica + audita origen=chat + enlaza mensaje; huella obsoleta → re-propone; `cancel`/"no" → cancelled, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 2

- [x] T011 [US2] Herramientas de **escritura** (`propose_set_day`, `propose_set_range`) + `confirm_pending`/`cancel_pending` (ofrecidas solo si hay proposed) en `apps/api/app/agent/tools.py`
- [x] T012 [US2] Orquestador: write → `AgentAction(proposed)` con preview + fingerprint; `confirm_pending` → re-valida fingerprint (re-propone si cambió) y aplica vía 003 con origen=chat enlazado; `cancel_pending` → cancelled, en `apps/api/app/agent/orchestrator.py`
- [x] T013 [US2] Guardrail de umbral: marcar `reinforced` si >14 días o variación >±25%, en `apps/api/app/agent/orchestrator.py`
...
- [x] T014 [P] [US3] Test: crear promo por chat propone→confirma→crea + re-publica + auditada, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 3

- [x] T015 [US3] Herramientas `propose_create_promotion`/`propose_delete_promotion` y su aplicación al confirmar (vía `promotion_service`), en `apps/api/app/agent/tools.py` (+ wiring en `orchestrator.py`)
...
- [x] T016 [P] [US4] Test: referencia relativa ("esos días") usa el rango previo del historial, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 4

- [x] T017 [US4] Incluir el historial de la conversación en el prompt (memoria) y soporte de referencias relativas, en `apps/api/app/agent/orchestrator.py`
...
- [x] T018 [P] [US5] Test: conversación usa `model_general`; escritura/confirmación usa `model_actions`; cambia con `LLMConfig`, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 5

- [x] T019 [US5] Enrutado de modelo (general vs. actions) leyendo `LLMConfig`, en `apps/api/app/agent/orchestrator.py`
...
- [x] T020 [P] [US6] Test: conversación con acción → historial recuperable y acción enlazada al mensaje, en `apps/api/tests/test_agent_orchestrator.py`

### Implementation for User Story 6

- [x] T021 [US6] Persistir mensajes (asistente/herramienta) y enlazar acción↔mensaje, en `apps/api/app/agent/orchestrator.py`
- [x] T022 [US6] Endpoint `GET /chat/stream` (SSE: eventos `token`/`tool`/`done`) envolviendo `run_turn`, en `apps/api/app/api/routes/chat.py`
...
- [x] T023 [P] Documentar los endpoints `/chat` y `/chat/stream` en `apps/api/README.md`
